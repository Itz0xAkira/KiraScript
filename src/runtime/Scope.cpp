		...
		const std::string& variableName, RuntimeValue* runtimeValue, bool constant) 
	{
		...
		if (variables.find(variableName) == variables.end())
			...
			throw Error(
				...
				2,
				"Cannot create kira " + variableName + " as it is already defined in this scope"
			);
		}
		...
		const std::string& variableName, RuntimeValue* runtimeValue) 
	{
		...
		if (variable.constant)
			throw Error(
				Error::Location::Interpreter,
				3,
				"Cannot redeclare kira " + variableName + " as it is a frozen kira."
			); 
	...
	Scope* Scope::resolveVariable(const std::string& variableName) {
		...
		if (parent == nullptr)
			throw Error(
				Error::Location::Interpreter, 
				4, 
				"Undefined kira " + variableName + " referenced in scope."
			);
